...
We see that the posterior draws do look like a beta density, and the sample mean and sample s.d. are almost the same as the theoretical values. Next, we calculate the proportion of sample draws of $\theta < 0.4$ and compare with the theoretical probability.
...
hist(pickvals, breaks = 50,main = "Draws from the posterior inverse chi-squared distr.",xlim = c(0,1))
...
We see that the two shapes are basically identical, which is what we derived in 2.a. Now, using these draws, the gini coefficient has posterior distribution proportional to the histogram below.
