...
# MCMC options
nIter <- 999 # Number of Gibbs sampling draws
...
plot(theta1, type ="l", ylim = c(10, 440))
t1ac <- acf(theta1[100:1000])
(sum(t1ac$acf[2:29])*2)+1 #the IF
...
ac <- acf(theta2)
(sum(ac$acf[2:31])*2)+1 #the IF

sigma1 <- c()
for(i in 1:1000){
  sigma1[i] <- sigma2[[i]][[1]]
}
plot(sigma1, type ="l", ylim = c(0, 8000))
sig1 <- acf(sigma1)
(sum(sig1$acf[2:31])*2)+1 #the IF

sigmaSecond <- c()
for(i in 1:1000){
  sigmaSecond[i] <- sigma2[[i]][[2]]
}
lines(sigmaSecond, col = "red")
sig2 <- acf(sigmaSecond)
(sum(sig2$acf[2:30])*2)+1 #the IF
...
(sum(p1$acf[2:29])*2)+1 #the IF

probObsInComp2 <- c()
for(i in 1:1000){
  probObsInComp2[i] <- probObsInComp[[i]][[2]]
}
lines(probObsInComp2, col ="red")
p2 <- acf(probObsInComp2)
(sum(p2$acf[2:30])*2)+1 #the IF
