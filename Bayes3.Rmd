...
Let check the convergence and the efficiency of all the parameters.
...
plot(theta1, type ="l", ylim = c(0, 90), main = "Traceplot of theta1")
t1ac <- acf(theta1[30:120], main = "ACF")
paste("IF: ",(sum(t1ac$acf[2:19])*2)+1) #the IF
```

$\mu_1$ seems to converge to a value around 20 but the draws seem to be highly positively correlated, which leads to a high IF, which means that the number of equivalent independent draws is merely a fraction of the actual draws.
...
plot(theta2, type = "l", main = "Traceplot of theta2")
ac <- acf(theta2[40:120], main = "ACF")
paste("IF: ",(sum(ac$acf[2:20])*2)+1) #the IF
```

$\mu_2$ seems to converge to a value around 100 but the draws seem to be positively correlated with draws an even number of lags apart.
...
plot(sigma1, type ="l", main = "Traceplot of sigma1")
sig1 <- acf(sigma1[20:120], main = "ACF")
(sum(sig1$acf[2:20])*2)+1 #the IF
```

$\sigma^2_1$ converges to a value close to 90, the draws seem to be highly positively correlated, which leads to a high IF, which means that the number of equivalent independent draws is merely a fraction of the actual draws.
...
plot(sigmaSecond, type = "l", main = "Traceplot of sigma2")
sig2 <- acf(sigmaSecond[20:120], main = "ACF")
(sum(sig2$acf[2:20])*2)+1 #the IF
```

$\sigma^2_2$ converges to a value close to 2100, the comment for the ACF and IF of $\sigma^2_1$ are true also in this case.
...
plot(probObsInComp1, type ="l", main = "Traceplot of pi1")
p1 <- acf(probObsInComp1[20:120], main = "ACF")
(sum(p1$acf[2:20])*2)+1 #the IF
```

$\pi_1$ converges to a value that is virtually zero and apart for the first 4 lags the draws seems to be independent.
...
plot(probObsInComp2, type="l", main = "Traceplot of pi2")
p2 <- acf(probObsInComp2[50:120], main = "ACF")
(sum(p2$acf[2:18])*2)+1 #the IF
```

Also $\pi_2$ converges to a value close to zero, but in this case from the ACF and IF we can see that the draws seem to be highly positively correlated.
...
We see that the plot shows that the gibbs sampler does make the regression coefficients converge to relatively small magnitudes after initially diverging from zero. We also see that the classification performance of the gibbs sampler coefficients is as good as the one obtained from coefficients found from the posterior mode normal distribution.
