...
theta <- vector("list", 120)
theta[[1]] <- quantile(x, probs = seq(0,1,length = nComp))
sigma2 <- vector("list", 120)
sigma2[[1]] <- rep(var(x),nComp)
probObsInComp <- vector("list", 120)
...
Let check the convergence of all the parameters.
...
p1 <- acf(probObsInComp1, main = "ACF")
...
p2 <- acf(probObsInComp2, main = "ACF")
